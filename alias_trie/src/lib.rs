...
mod tests {
    ...
    fn test5() {
        ...
        assert_eq!(value, UniqueOption::Some(&"first/Output![Demo6A,0].0$".to_string()));

        let value = trie.get(&["first/Output!", "1$"]);
        assert_eq!(value, UniqueOption::Some(&"first/Output![Demo6A,0].1$".to_string()));

        let value = trie.get(&["self", "b", "Double$"]);
        assert_eq!(value, UniqueOption::Some(&"self.b[Demo6B,0].Double$".to_string()));
        ...
        assert_eq!(value, UniqueOption::Some(&"self.b[Demo6B,0].Double[Demo6A,1].1$".to_string()));
    }
}
